...
try:
    ...
    if vmin_files:
        ...
        # Calculate center of other_hw points
        kmeans = KMeans(n_clusters=1, random_state=42).fit(filtered_df_other_hw[['delta_p25', 'x_iqr']])
        center = kmeans.cluster_centers_[0]

        # Compute distances of with_hw points to the center
        distances = cdist(filtered_df_with_hw[['delta_p25', 'x_iqr']], [center])
        threshold = 4.5 * np.std(cdist(filtered_df_other_hw[['delta_p25', 'x_iqr']], [center]))  # Example threshold

        # Identify outliers
        filtered_df_with_hw['outlier'] = distances.flatten() > threshold
        ...
        outliers = filtered_df_with_hw[filtered_df_with_hw['outlier']]
        plt.scatter(outliers['delta_p25'], outliers['x_iqr'], edgecolor='blue', facecolor='none', s=100, label='Outliers')
        plt.scatter(center[0], center[1], color='black', marker='X', alpha=0.75, s=150, label='Cluster Center')
