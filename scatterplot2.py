...
try:
    ...
    if vmin_files:
        ...
        centroid_with_hw = filtered_df_with_hw[['delta_p25', 'x_iqr']].mean()
        centroid_other_hw = filtered_df_other_hw[['delta_p25', 'x_iqr']].mean()
        print(centroid_with_hw)
        print(centroid_other_hw)
        plt.scatter(centroid_with_hw[0], centroid_with_hw[1], color='black', marker='X', alpha=0.75, s=150, label='Cluster Center')
        plt.scatter(centroid_other_hw[0], centroid_other_hw[1], color='black', marker='X', alpha=0.75, s=150, label='Cluster Center')

        distance = np.linalg.norm(centroid_with_hw - centroid_other_hw)
        print(f"Distance between centroids: {distance}")

        combined_data = pd.concat([filtered_df_with_hw, filtered_df_other_hw])
        features = combined_data[['delta_p25', 'x_iqr']]
        ...
        print(combined_data.groupby(['cluster', 'hw_name']).size())
        ...
        plt.scatter(combined_data['delta_p25'], combined_data['x_iqr'], c=combined_data['cluster'], cmap='viridis', alpha=0.5)
        ...
        if len(unique_clusters) > 1:
            ...
            is_downshift = False
        print(f"is_downshift: {is_downshift}")
